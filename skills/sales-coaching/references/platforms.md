# Platform-Specific Coaching Guide

Detailed coaching guidance for each enablement, conversation intelligence, and QA platform — what it does for coaching and how to use it effectively.
...
- [MindTickle](#in-mindtickle)
- [SalesHood](#in-saleshood)
...
- **Real-time coaching during calls**: Balto delivers live prompts, objection handling suggestions, compliance checklists, and next-best-action recommendations to agents during calls — sub-200ms latency. This is coaching during the call, not after.
- **Dynamic playbook builder**: Managers can update playbooks and guidance rules in real-time without IT involvement. Changes propagate instantly to all agents.
- **Automated QA scoring**: Balto scores 100% of calls against custom criteria (95% accuracy claimed). Supervisors see calls ranked by coaching priority without manual sampling.
- **Coaching moments**: AI identifies specific call segments that need coaching attention, links them to guidance adherence data, and suggests which agents need intervention.
- **Compliance monitoring**: Real-time PCI/HIPAA/regulatory scanning with PII scrubbing. Compliance alerts trigger during calls, not discovered after.
...
- **InstaScore 100% automated QA**: Level AI's core coaching value is automated quality assurance on every interaction using custom scorecards. Moves QA from manual sampling to 100% coverage. Managers coach on data from InstaScore — not 2-3% random samples.
- **Custom QA scorecards**: Define weighted evaluation criteria (compliance, verification, empathy, resolution, script adherence). InstaScore auto-scores every interaction against these criteria with near-human accuracy using the QA-GPT model.
- **Coaching from VoC insights**: Voice of Customer analytics (iCSAT, sentiment, intent detection) feed coaching priorities — "Customers are increasingly frustrated about billing — let's train agents on the updated billing FAQ."
- **Screen recording for context-rich coaching**: Agent desktop capture alongside call audio provides full picture for coaching sessions. See what agents see during calls, not just what they say.
- **AgentGPT real-time coaching**: Real-Time Agent Assist surfaces knowledge articles and next-best-action during live calls — coaching happens in-the-moment, not just post-call.
- **Limitations**: Mid-market focus (100-1,000 agents). No public pricing (~$185/agent/month). API not publicly documented. Call ingestion can be delayed 24+ hours, making same-day coaching difficult. No AI roleplay. No methodology scoring (MEDDIC/BANT/SPICED). For sales-deal coaching, use Gong or Sybill. For faster deployment, try Balto.
- **Best practice**: Use Level AI coaching for mid-market contact centers wanting combined QA + real-time assist in one platform. The semantic intelligence approach (intent-based, not keyword) means scorecards can capture nuance better than rule-based alternatives. Start with 5-8 binary scorecard criteria and calibrate against manual QA for 2-4 weeks before trusting auto-scores for coaching decisions.

### In ExecVision (Mediafly Coach360)

- **Coaching plans from AI-identified gaps**: ExecVision's core differentiator is translating call data into structured coaching plans. AI analyzes calls to identify performance gaps, then front-line managers create individualized plans based on recommendations. The workflow is: analyze calls → identify gap → create coaching plan → link specific call clips → track improvement.
- **Smart alerts for coachable moments**: Configurable alerts fire when specific keywords, phrases, or conversation patterns appear. Set up alerts for coaching opportunities (competitor mentions, pricing objections, weak discovery) and get notified in real-time or on trend basis. More useful for proactive coaching than post-hoc review.
- **Performance dashboards + self-review**: Reps can compare their own conversation activity to team averages — talk ratio, question count, keyword usage. Self-awareness drives behavior change before manager intervention. Managers see team-wide trends and drill into individual rep patterns.
- **Conversation library for onboarding**: Searchable repository of call clips organized by type (discovery, demo, close, objection handling). Curate winning examples into playlists for new rep onboarding. More effective than generic training content because it uses your team's actual conversations.
- **Broad ingestion (59+ integrations)**: Ingests from 38 dialers/telephony systems, 9 conferencing platforms, and file storage (S3, Box, Dropbox, Drive). One of the widest integration footprints in CI — most recording sources are supported natively.
- **Limitations**: Acquired by Mediafly in 2022 — now Coach360 inside Revenue360 suite. No longer sold standalone, no public pricing (historically ~$100-185/user/month). API exists but not publicly documented. Filter UI is unintuitive (top G2 complaint). No AI roleplay. No methodology scoring (MEDDIC/BANT/SPICED). For deal intelligence and pipeline coaching, use Gong. For affordable CI, try Rafiki ($19/user/mo) or Avoma ($19/seat/mo).
- **Best practice**: Use ExecVision coaching when your primary need is structured coaching plans tied to call evidence, not just call analytics dashboards. The platform excels at the "translate insight into action" step that many CI tools skip. Build coaching plans around specific call clips with timestamps — "watch this 2-minute clip and notice how the rep didn't ask about budget" is more impactful than "your discovery score is low." Update conversation library quarterly to keep examples fresh and relevant.

### In Rafiki

- **Smart Call Scoring with methodology frameworks**: Rafiki's Coaching Agent auto-scores every call against MEDDIC, BANT, SPIN, or GAP frameworks. Managers see scores per call and trends over time without manually reviewing recordings. Custom scoring criteria available — adjust weights and criteria to match your actual sales process.
- **AI Role Play Agent**: AI-powered practice scenarios where reps practice against simulated buyer personas and receive instant scoring on five dimensions. Available without needing a live partner — reps can practice independently on their own schedule.
- **Coaching playlists and conversation library**: Curate call clips and full recordings into coaching playlists. Organize by topic (discovery, objection handling, closing) for onboarding and ongoing skill development. Tag moments with coaching notes.
- **Gen AI insights for coaching priorities**: Ask Rafiki (natural language search across all calls) surfaces team-wide patterns — which objections are reps struggling with? Which methodology steps are consistently missed? Use insights to prioritize coaching topics.
